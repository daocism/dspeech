  ...
  func run() async {
    ...
    if status == .authorized, let recognizer {
      ...
      for path in paths {
        ...
        do {
          let transcript = try await Self.transcribeWithSimulatorFallback(
            url: url,
            recognizer: recognizer
          )
          recognized.append(
            Result(fixture: url.lastPathComponent, transcript: transcript, error: nil))
        } catch {
          recognized.append(
            Result(
      ...
      results = paths.map { path in
        Result(fixture: URL(fileURLWithPath: path).lastPathComponent, transcript: nil, error: error)
      }
    }
    ...
    do {
      ...
      let fallback =
        #"{"locale":"en-US","requiresOnDeviceRecognition":true,"authorizationStatus":"writeFailed","recognizerAvailable":false,"supportsOnDeviceRecognition":false,"results":[]}"#
