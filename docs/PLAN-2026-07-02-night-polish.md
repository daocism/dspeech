...
- [x] B5. Shared `AVAudioEngineTapSession` primitive encoding the `format: nil` + `@Sendable` tap tribal knowledge once; migrate `LiveAudioCaptureConduit`.
- [x] B6. Migrate the other 3 capture wrappers (CallsignDictation, VoiceEnrollment, InputLevelMeter) to the shared tap primitive.
...
- [x] C3. Download pause/resume UX in Settings (builds on C1; `cancelDownload` already exists).
...
- [x] C6. Richer transcript export: timestamped `.txt` + structured `.jsonl` share options in session history detail.
- [x] C7. Session summary metadata surfaced: duration, engine used, recognition locale in history rows + detail header.
- [x] C8. Transcript storage usage surfaced in Settings + opt-in auto-cleanup of old sessions (default OFF, honest copy).
...
- [x] C10. Parakeet confidence-0 / VERIFY-badge behavior review: keep honest, avoid badge noise (documented decision in code, no product-semantics change).
...
- [x] D11. Reduce-Motion audit: honor `accessibilityReduceMotion` in ALL remaining `withAnimation` sites.
...
- [x] E1. Real iPad shell: `NavigationSplitView` (transcript + sidebar) replacing the letterboxed phone layout.
- [x] E2. Settings/history as sidebar panes on regular width.
- [x] E3. Landscape refinements; revisit the 720pt cap inside the split layout.
- [x] E4. iPad UI tests updated for the split shell.

## Phase F — Tests (14)
- [x] F1. Split `VoiceFilterTests.swift` (3200+ lines, 15 structs) into per-subsystem files, zero behavior change.
...
- [x] F7. Snapshot suite: transcript cards × Dynamic Type {default, AX-XXXL} × {en, de}.
- [x] F8. Snapshot suite: chips/badges/banners matrix (post-D14 two-tier system).
- [x] F9. UI test: scripted live transcript + translation enabled simultaneously.
- [x] F10. UI test: permission denied → Settings deep-link → re-request flow.
- [x] F11. Add `ru` sweep to AccessibilityAuditUITests (shipping default test locale is never audited today).
- [x] F12. Dark-lock consistency UI test under system light mode (ui-quality light+dark requirement, honest for a dark-locked app).
- [x] F13. PBT (PropertyBased): installer state machines — checksum mismatch / disk-full / resume branches.
- [x] F14. VoiceOver affordance for transmission-card tap-to-expand (`accessibilityAction`) + UI test.
...
- [x] G6. Release-policy/check-release-ready updated for the new SwiftPM deps (pins validated).
- [x] G7. Pre-push hook: device-arch compile gate (proven to catch Swift-6 errors the sim lane misses).
- [x] G8. CI uploads snapshot-test failure artifacts (diff images) on failure.
...
- [x] H12. Post-glass longest-locale re-verification (German AX sweep via existing UI tests).
...
- [x] I3. Capture App Store screenshots (simulator, en + top locales) via `scripts/screenshots/`, review every frame with own eyes.
...
- [x] I5. Run `xcrun privacyreport` on a fresh unsigned archive — FluidAudio/WhisperKit SDK privacy-manifest audit (open item from nutrition-labels mapping).
...
- [x] I8. README refresh — honest current product/engineering state.
- [x] I9. `docs/ai-kb/current-context.md` + `.ai/project-state.md` updated post-mission.
...
- [x] J1. Full local suite (unit + core UI), zero warnings, device-arch compile gate.
- [x] J2. `DspeechFull` a11y/Dynamic-Type/multi-locale sweep locally, incl. Reduce-Transparency additions.
