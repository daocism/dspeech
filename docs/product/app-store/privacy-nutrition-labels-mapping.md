...
| FluidAudio SDK privacy manifest and tracking audit | Not declared in this app manifest | `Dspeech.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved` pins `FluidAudio` `0.14.7` from `https://github.com/FluidInference/FluidAudio.git`. | Block App Store "Data Not Collected" publication until the processed archive confirms no SDK-collected data, no tracking domains, and no additional required-reason APIs beyond this worksheet. | Dspeech does not intentionally collect audio, transcripts, voice samples, route metadata, or identifiers through FluidAudio. The remaining question is the SDK/archive disclosure surface, not app product intent. | Run `xcrun privacyreport` or Xcode Organizer's privacy report on the final archive and attach the result before Andrei approves privacy answers. North-star §4 requires every third-party SDK/binary dependency to be audited. |
