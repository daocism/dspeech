...
private let locales: [String?] = [nil, "en-US", "fr-FR"]

private let urgencyPhrases = [
  "MAYDAY", "PAN PAN", "PANPAN", "SECURITE", "SÉCURITÉ", "ALL STATIONS",
]

private func randomScore(using rng: inout SeededGenerator) -> Float {
  Float(Int.random(in: 0...1000, using: &rng)) / 1000
}

private func randomNonPilotOrMixed(using rng: inout SeededGenerator) -> SpeakerMatchDecision {
  Bool.random(using: &rng)
    ? .nonPilot(bestPilotScore: randomScore(using: &rng))
    : .mixed(bestPilotScore: randomScore(using: &rng))
}

private func randomAnySpeaker(using rng: inout SeededGenerator) -> SpeakerMatchDecision {
...
private func injectUrgency(into text: String, using rng: inout SeededGenerator) -> String {
