...
struct SpeakerModelPackInstallerTests {
    ...
    @Test func productionManifestPinsFluidAudioModelFiles() {
        let manifest = Dictionary(uniqueKeysWithValues: SpeakerModelPackInstaller.expectedModelFileManifest.map {
    ...
    @Test func verifierRejectsChangedBytesWithSameFileNamesAndSizes() throws {
        ...
        do {
            ...
            guard case let .integrityChecksumMismatch(relativePath, expectedSHA256, actualSHA256) = error else {
    ...
    private static func manifest(for contents: [String: Data]) -> [SpeakerModelPackInstaller.ExpectedModelFile] {
        contents
            .map { SpeakerModelPackInstaller.ExpectedModelFile(relativePath: $0.key, sha256: Self.sha256($0.value)) }
